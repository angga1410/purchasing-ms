	...
	<div id="container">
		<div id="header">
			...
			<div id="reference">
				...
				<h4>PO#. : {{ $data->po_number }}{{ $data->po_number_seq }}</h4>
				<p>Date Report : {{ date("Y-m-d H:i:s")}} </p>
			</div>
		</div>
		...
		<div id="fromto">
			...
			<div id="to" >
			<p value="{{ $data->po_number }}"  onchange="location = this.value;"></p>
				<p>
					<strong>VENDOR NAME</strong><br>
					@foreach( $supplierContacts as $get )
										@if( $get->id == $data->supplier_contact_id )
										<strong>{{ $get->contact_name }}</strong><br>
										@endif
									@endforeach
				
									@foreach( $supplierContacts as $get )
										@if( $get->id == $data->supplier_contact_id )
										<strong>{{ $get->contact_hand_phone }}</strong><br>
										@endif
									@endforeach
									@foreach( $supplierContacts as $get )
										@if( $get->id == $data->supplier_contact_id )
										<strong>{{ $get->contact_email }}</strong><br>
										@endif
									@endforeach
				</p>
			</div>
